...
function formulaLifecycle(car) {
  ...
  return Math.max(
    ...
    Math.min(
      15,
      base - agePen - mileagePen + maintEffect - roadPen - accPen + hwBonus
    )
  );
}
...
async function setupSchema(client) {
  ...
  await client.query(
    `CREATE INDEX IF NOT EXISTS cars_life_idx ON cars (ml_remaining_life_yrs DESC NULLS LAST)`
  );
}
...
function toCSV(rows) {
  const cols = [
    ...
    'fuel_type',
    'engine_cc',
    'ml_remaining_life_yrs',
    'formula_remaining_yrs',
    'lifecycle_category',
  ];
...
async function main() {
  ...
  try {
    ...
    // 4. Train
    console.log(
      '4. Training TensorFlow.js model (4 000 samples, 120 epochs)...'
    );
    ...
    // 7. Stats
    const avg =
      results.reduce((s, r) => s + r.ml_remaining_life_yrs, 0) / results.length;
    const cats = {};
    results.forEach((r) => {
      cats[r.lifecycle_category] = (cats[r.lifecycle_category] || 0) + 1;
    });
    ...
    for (const rs of ROAD_SEGMENTS.sort(
      (a, b) => b.road_quality - a.road_quality
    )) {
      console.log(
        rs.region.padEnd(22) +
    ...
    fs.writeFileSync(
      roadPath,
      JSON.stringify({ road_segments: ROAD_SEGMENTS }, null, 2),
      'utf8'
    );
